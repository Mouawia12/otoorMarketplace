...
export default function AdminReportsPage() {
  ...
  const fetchReports = async () => {
    setData([]);
    setSummary({ totalSales: 0, totalCommission: 0, totalAuctions: 0 });
    setLoading(false);
  };
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-luxury p-6 shadow-luxury">
        ...
        <div className="mb-6">
          ...
          {data.length === 0 ? (
            <div className="text-center py-12 text-taupe">{t('admin.noReportData', 'لا توجد بيانات حالية')}</div>
          ) : (
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {data.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-taupe">
                    {t('admin.noReportData', 'لا توجد بيانات حالية')}
                  </td>
                </tr>
              ) : (
                data.map((d) => (
